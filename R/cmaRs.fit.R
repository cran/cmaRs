...
# this function prepares the data for cmars modeling, takes the BFs from MARS, 
# constructs a CMARS models and then calculates the performance measures
...
formula <- eval(as.formula(paste(response.name,"~", pred.names,  sep = "")))
...
model_mars <- earth(formula, data = data, degree = degree, 
          nk = nk, glm = list(family = binomial), pmethod="none", 
          trace=4, Auto.linpreds = Auto.linpreds) 
}else{
# constructs only the backward step of MARS model
...
{
istr <- as.character(i)
 bfs_forward_names <- str_replace_all(bfs_forward_names, 
...
{
istr <- as.character(i)
colnames_vector <- c(colnames_vector, (text <- paste("xfirst", istr, sep = "")))
}
...
# replace "--" signs with "+" 
bf.vector1 <- str_replace_all(bf.vector, "--", "+")
# assign vector of BFs to variable bf.vector.  
bf.vector <- bf.vector1
bf.vector1 <- bf.vector
# delete "h" in BFs.
bf.vector1 <- str_replace_all(bf.vector, "h\\(", "\\(") 
bf.vector <- bf.vector1
...
for (i in 1 : numBF)
{
  son <- str_replace_all(bf.vector, "\\)", "")
  son1 <- str_replace_all(son, "\\(", "")
  son2 <- str_replace_all(son1, "\\(", "")
  c <- son2
  c.new <- c
  c.new1 <- str_replace_all(c.new, 'first', '')
  c.new2 <- paste('pmax(0,', c.new1, ')', sep = "")
  c.new3 <- str_replace_all(c.new2, "\\*", "\\)*pmax(0,")  
# obtain derivative for each BF  
  r <- int[i]  
  if (r == "m")
  { # write the BF in general format for main effects such as max(0,x2-3)
    b1 <- c[i]
...
b1 <- paste(b1, '*', bb, sep = "")
 ...
 bf.cmars <- c.new3
  # construction of BF matrix as a symbol to take symbolic derivative
  yacas(Sym(b1)) # built-in function yacas uploaded for symbolic derivatives
  BF <- yacas("Simplify(%)") # reduce the expression to a simples form  
  for (j in 1 : numBF)
  {      
    istr <- as.character(j)
...
# taking derivatives
  ...
  elm.dms <- Sym(0)
  DMS <- matrix(elm.dms, nrow <- p,ncol <- p) # symbolic derivative matrix (DM)
  elm.varms <- Sym(1)
...
M5 <- cbind(matrix(0, nrow <- numBF, ncol <- 2), L[2:(numBF+1), 
      2:(numBF+1)], matrix(0, nrow <- numBF, ncol <- n))
...
for (i in 1 : s)
{
optimization_all <- cmaRs.mosek_optimization(i, numBF,
...
if (classification == TRUE)
{
auc.value <- c(auc.value, auc(roc(as.numeric(optimization_all$m),
...
final_opt <- cmaRs.mosek_optimization(1, numBF, sqrtz.final,
...
bf.vector1 <- str_replace_all(bf.vector, "h\\(", "\\(") 
bf.vector <- bf.vector1
bf.vector <- str_replace_all(bf.vector, "\\(", "\\pmax(0,")
bf.vector <- str_replace_all(bf.vector, "\\)\\)", "\\)")
bf.vector <- str_replace_all(bf.vector, "0,\\(", "0,")
bf.cmars <-  bf.vector
...
# returned fields match cmaRs's fields
   retval <- list(coefficients  = final_Theta,
               formula = formula,
